...
@property (nonatomic,strong) UIButton *autoRead;
@end
...
-(UIButton *)autoRead{
    if (!_autoRead) {
        _autoRead = [LSYReadUtilites commonButtonSEL:@selector(autoReadClick:) target:self];
        [_autoRead setTitle:@"自动翻页" forState:UIControlStateNormal];
    }
    return _autoRead;
}
...
- (void)autoReadClick:(UIButton *)autoBtn{
    if ([self.delegate respondsToSelector:@selector(autoReadAction:)]) {
        [self.delegate autoReadAction:@(DefaultAutoReadSpeed)];
    }
}
...
-(void)layoutSubviews
{
    ...
    _autoRead.frame = CGRectMake(ViewSize(self).width - 50 - 80, 24, 100, 40);
}
/*
